...
def run_single_experiment(seq_length, alphabet_size, seed):
    ...
    alphabet_seen = np.random.choice(
            alphabet, size=3, p=source_probs, replace=False)
    ...
    sequence = np.r_[alphabet_seen, np.random.choice(
            alphabet_seen, size=seq_length-3, p=source_probs_seen)]
   

    source_probs=np.array([0.2480172 , 0.44118351 ,0.31079929])
    sequence=np.array([1,2,0,2,1,1,1,0])
    ...
    symbol_codec = codecs.Categorical(source_probs, prec=27) # returns codecs.categorical.encode and cs.categorical.pop
